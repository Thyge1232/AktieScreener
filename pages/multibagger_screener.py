...
from config_loader import load_config, load_region_mappings

# --- Konfiguration (Korrekt placering) ---
st.set_page_config(page_title="Multibagger Screener", layout="wide")
config = load_config()
...
st.markdown("Upload en Finviz CSV-fil og vælg dine screeningskriterier.")

# Fjern de to fejlagtige linjer herfra
# config = load_config() # <-- Slettet
# df_results = screen_stocks(...) # <-- Slettet
...
with col2:
    # VIGTIGT: Sørg for at selected_profile_name har en default værdi
...
dynamic_weights = {}
# ... (resten af din kode til at definere dynamic_weights er korrekt)
...
if uploaded_file is not None and selected_profile_name:
    with st.spinner("Behandler data..."):
        ...
        if df_raw.empty:
            ...
            # ========================================================================
            # == DEN KORREKTE LINJE ER HER ==
            # Tilføj 'config' som det tredje parameter
            ...
            # ... resten af din fil er korrekt og uændret ...
            ...
            if not df_results.empty:
                ...
                st.download_button(label="Download fulde resultater som CSV",data=csv_result,file_name=f'multibagger_results_{selected_profile_name}.csv',mime='text/csv',)
            else:
